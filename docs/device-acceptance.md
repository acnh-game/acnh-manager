...
- 入口两处:`+` 键,以及页眉右侧新增的 `+ 说明` 入口(与头部标签共用 `LayoutHeaderTabs`,打开
  说明页时它自己高亮);
- 内容:标题 + 四行说明(聊天码是物品编号的十六进制写法;`/9c9` 会生成"金矿"到背包第一个槽位;
  一行可写多个、空格分隔、最多 10 个并给出 `/9c9 9c6 9c4` 的例子;编号从《森友物码册》小程序查)
  + 小程序码(中间白区里写着“物码”两个字,与原码 logo 同位置);
- 排版两处实测修正:说明变四行后,固定 7px/模块的二维码会**压进页脚**(实测墨迹到 y=657,页脚从 ~648 开始),
  于是模块尺寸改为按剩余空间自适应(`clamp(可用高度/(模块数+2), 4, 7)`),并收紧卡片留白 —— 现在
  模块回到 7px(259px 见方)、距页脚 14px;“物码”两个字先按行盒居中时墨迹偏高 4px,加了 5px 下沉偏置后
  实测墨迹中心 (205.5, 506) 对二维码中心 (206, 505),偏差 ≤1px;
