...
| 旧金手指共存 | 卡上保留 `cheats/FF1D1C05670DB602.txt`(真机文件名,内含 15 条) | **首页**状态行下方出现琥珀色警告条并写出**绝对路径**;详情页保留同一行;文件里只有无关条目时不提示 | 提示本身已验(2026-09-23,见文末"旧金手指冲突"一节;此前这条从未触发过,因为判据写错了文件名形状)。"文件里只有无关条目不报"与"条目改名仍能认出"两条**只有主机用例覆盖**,真机复验要临时改写卡上那份金手指文件,尚未做 |
...
- 入口两处:`+` 键,以及页眉新增的 `＋ 功能说明` 入口(与头部标签共用 `LayoutHeaderTabs`,打开
  说明页时它自己高亮);页眉从左到右为 `Ⓛ 状态 / Ⓡ 详情 / ＋ 功能说明`;
...
- **扫码实测(2026-09-22)**:用户用手机扫屏幕上这个码,正常打开《森友物码册》小程序 —— 采样把 logo
  圆盘区域涂白(面积与原码被 logo 遮住的相同)不影响解码,说明页上那两个字标也没有越过消隐范围。

## 旧金手指冲突:检测从未触发与修复(2026-09-23)

现象:卡上一直躺着 `atmosphere/contents/01006F8002326000/cheats/FF1D1C05670DB602.txt`(6 118 B),
但首页、详情页都没有任何冲突提示,`log.txt` 里也从来没有 `legacy cheat present` 那一行 ——
用户实测反馈"manager 应该能检测到旧 cheat 文件,但机器上没看到提示"。

根因是**文件名判据写错了形状**。旧代码要求 `name.size() == 36`,即 32 个十六进制字符加 `.txt`;
而 Atmosphere 真正载入的金手指文件取的是运行中主模块号**前 8 字节**:

```c
sdmc:/atmosphere/contents/%016lx/cheats/%02x%02x%02x%02x%02x%02x%02x%02x.txt
```

(`stratosphere/dmnt/source/cheat/impl/dmnt_cheat_api.cpp` 的 `CheatProcessManager::LoadCheats`),
也就是 **16 位十六进制 + `.txt` = 20 字符**。真机卡上的名字正好 20 字符,于是那个条件永远为假:
这段代码从 2026-09-17(`e6e4a6b`)进仓库起,一次都没有命中过。

修复:

- 判据抽成不依赖 libnx 的 `source/env/cheat_file.*`(`IsLegacyCheatFileName`,16 位十六进制 +
  `.txt`,大小写都认 —— 卡是 FAT/exFAT,dmnt 用小写查、卡上是全大写也能命中),`detect.cpp` 改用它;
- **判据从"文件存在"改成"文件里确有条目"**:只要文件名对上就报警,会把整包无关金手指也算成冲突。
  真机卡上那份文件有 15 条,其中只有 `[聊天码生成到背包(最多10个)(X+十字右)]` 走聊天码通路,
  其余 14 条(体力值、复制槽位、清空、里程数)与 agent 无关,让用户为它们删掉整个文件是误伤。
  条目怎么认:`ChatCodeCheatEntry` 找**同时**读 `main+0x05255A60`(文字输入对象)与
  `main+0x5474040`(玩家链)的条目 —— 用代码而不是名字当身份,因为玩家会改条目名,而且名字里
  还带着激活键与个数,两样都会随指南版本变(地址语义见根目录
  `docs/text_spawn_to_slot1_guide.md` 的"关键地址")。成对判据是必要的:同一份文件里
  `[里程数最大 9999999]` 也读玩家链,但不读文字输入对象,只认一个地址就会把它误报;
- 主机侧新增 `TestCheatFile`(19 条):文件名 10 条(含卡上真实名字、dmnt 的小写写法、
  `toggles.txt`、旧的 36 字符形状哨兵)+ 内容 9 条(整包、改名后的条目、注释/花括号/CRLF/乱空格、
  带 UTF-8 BOM 的文件、只有玩家链的里程数条目、只有文字输入对象的条目、空文本、无条目)。
  带 BOM 那两条是实测补的:解析器原来会把 BOM 开头的第一行当成非条目行,于是**丢掉那一条的代码**;
  记事本默认就写 BOM,玩家改一下这份 txt 就能触发;
- 界面位置同时调整:这类漏看就会出错的消息移到**首页**状态行下方(琥珀色警告条),
  详情页保留同一行,理由见 `docs/architecture.md` §7 与 `AGENTS.md` 的文案原则;
- 警告条里给出的**是绝对路径而不是文件名**(`/atmosphere/contents/01006F8002326000/cheats/…`):
  只给文件名,屏幕上没有任何东西告诉玩家它在哪里;路径长了整整 42 个字符,于是这一行降到
  `kFontSmall`(18),`kHomeWarnHeight` 也随之由行高推导(= 行高 + 上下各 18),按钮位移
  `kHomeWarnShift` 同样由警告条几何算出来,不再是写死的 52;
- 警告三角按等边警告三角的比例画(底边比高多 1/5,`!` 在三角形内部居中):底边两倍于高度的
  画法在电视上是一条横向拉伸的形状,这个尺寸关系只能在真机上看出来;
- 文案说的是**代码**,不是文件:"检测到可能冲突的旧金手指代码" —— 文件躺在卡上不是问题,
  判据本身也只认带那条代码的条目;
- 配色是**琥珀橙**(`kWarn`,色相约 40°)而不是红:边框与三角用更亮的 `kWarnBright`(纯装饰),
  文字用 `kWarn`,底色 `kWarnTint` 是淡黄而不是偏红的浅桃色。红只留给"操作失败"(`kBad`) ——
  这一条同时写进了 `AGENTS.md` 的视觉语义;
- 详情页因此多了一行,把**英文页面的老毛病顶了出来**:英文详情页本来只差约 10 px,加行后下方
  "高级"卡片被切掉半行。按这个页面已有的收紧阶梯补了最后一档 —— 行距 8 → 0、底部留白 22 → 16
  之后,整页降一档字号(`kFontSmall`),而不是让卡片被裁;中文仍走名义档位(字号不变),英文落到
  最后一档(每一行都在)。

真机验证(2026-09-23,`acnh-manager.nro` sha256 `7d5ce8e3…`,
构建戳 `2026-09-23T04:49:20Z+80ec96f-dirty+src:5ef16658`;提交前的最后一轮只是注释与写法整理,
下面的测量数字在同一构建上复现过):

- 首页:状态行下方出现警告条
  `⚠ 检测到可能冲突的旧金手指代码  /atmosphere/contents/01006F8002326000/cheats/FF1D1C05670DB602.txt`,
  英文界面为 `Found an old cheat code that may conflict  /atmosphere/contents/…/FF1D1C05670DB602.txt`
  —— 两种语言都一行排下,右侧余量分别约 140 px 与 90 px(路径 62 字符是这一行的长度来源);
  它**与安装状态无关**:未安装时("还没有安装 agent")与已安装时("agent 已安装,可以使用")
  都在同一位置出现,因为它讲的是卡上的金手指代码,不是 agent 的状态;
- 详情页:安装信息卡片末尾多出一行 `旧金手指代码  /atmosphere/contents/01006F8002326000/cheats/FF1D1C05670DB602.txt`
  (警告色),专业信息没丢,卡片高度仍是一屏放得下;
- 详情页(英文):中文走名义档位(字号不变),英文自动落到最后档 —— 两张卡片的每一行都在,
  下边框完整落在页脚之上,不再出现被切掉半行的情况;
- 警告条出现时两排按钮整体下移(现为 `kHomeWarnShift` = 46 px,由警告条几何算出),
  触控目标与画面一起移动(动作表与渲染共用同一组矩形);
- 语言切换后两页文案都跟着变,切回中文后设置保持;
- 判据的目录范围:把 `cheats/` 整个改名成 `cheats_tmp` 再重启 → 警告条消失;改回 `cheats` 再重启
  → 警告条回来。dmnt 只载入 `cheats/`,玩家用改名的办法关掉金手指时不该留下假警告,这条行为符合预期;
- 内容判据:卡上那份 15 条的文件仍然报(冲突条目还在),`log.txt` 的新形状为
  `legacy cheat present: /atmosphere/contents/01006F8002326000/cheats/FF1D1C05670DB602.txt entry="聊天码生成到背包(最多10个)(X+十字右)" (same chat path as the agent)`;
  "条目改名仍能认出""只留无关条目不报"两条由主机用例覆盖(`TestCheatFile` 的 9 条内容用例),
  在真机上做这两步要临时改写卡上那份金手指文件,未获授权前不动用户的卡;
- 文字与三角的对齐:警告条文字**不再套用按钮的 `kLabelOpticalBias`**(那个偏置是给 110 px 按钮里
  的标签校的)。同一张截图、同一把尺子量前后两次(JPEG 振铃用 `--tol 30` 压掉,条中心 266):

  | | 三角墨迹中心 | 文字墨迹中心 | 文字相对三角 |
  |---|---|---|---|
  | 加偏置(旧) | 265.5 | 270.0 | 低 4.5 px |
  | 不加偏置(现) | 265.5 | 264.0 | 高 1.5 px |

  1.5 px 属于这把尺子在压缩截图上的噪声量级,肉眼等价于居中;还想再压的话就是给这一行单独加一个
  +2 的偏置,但那属于对单次测量过拟合,没做。

## 启动日志与界面各说一套(2026-09-23,已修)

`log.txt` 开头那段诊断块的最后两行,在发布导入出现之前就被写死了 —— 于是每个真实构建的日志都写着

    gate: no-manifest (no manifest available)
    manifest: no embedded release manifest (waiting for the release import tool)

而同一个构建的详情页写的是 `检测结果: supported build v3_0_3 (3.0.3)`、`清单来源: 随应用自带`。
根因:`ReportEnvironment` 用 `Evaluate(nullptr, …)` 求门控,并硬编码了"没有内嵌清单"那句话。报障时
要读的正是 `log.txt`,所以这不是措辞问题,而是日志在说假话 —— 构建明明带着发布物,它却说没有。

修法照界面的做法来:解析内嵌发布物(`payload::EmbeddedAvailable()` + `manifest::Parse`)再 `Evaluate`,
并打印这份清单的 agent 版本与 commit;没有导入发布物、或清单被校验拒掉时,如实写出原因。联网检查
在后面才跑、可能采用更新的清单,那一份由详情页与更新检查那两行负责,启动这两行只讲"这个构建带了什么"。

真机验证(构建戳 `2026-09-23T04:53:50Z+80ec96f-dirty+src:32e690c8`):
...
## 商店截图与指南页真机截图(2026-09-23)

商店条目与指南页要的截图都在这次会话里从真机抓取,用的是卡上那份 1.0.1 发布件
(`/switch/ACNH-Manager/acnh-manager.nro` sha256 `3ad1c969…`,与 `packaging/nro/acnh-manager-1.0.1.nro`
逐字节相同),所以图里的界面与商店分发的构建一致。

| 用途 | 文件 | 画面 | 语言 |
|---|---|---|---|
| 商店条目 ×4 | `assets/screenshots/{status,install,done,uninstall}.png` | 首页(已安装)/ 安装确认页 / 安装成功页 / 卸载确认页 | English |

过程:起始状态是"还没有安装 agent"(`exefs/` 不存在)→ 详情页按 A 切到 English → 首页 A 进确认页 →
A 安装 → 结果页 → A 完成(首页变成"已安装")→ 按 Y 打开卸载确认页 → 按 B 返回 → 详情页按 A 切回中文。
抓完卡上 `settings.json` 仍是 `{"schema":1,"language":"zh-Hans"}`,`exefs/` 里留着刚安装的三个文件
—— 起始状态与此不同,当时卡上没有 agent。

商店那四张原来是 JPEG 数据、文件名却是 `.png`;这次一并转成真 PNG(上游 196 个截图资产全部是
`.png`)。同一次也抓过两张"还没有安装 agent"的首页画面(中英各一)给指南页,后来决定指南页不放
截图(正文说清楚就够了),那两张没有保留。
