...
| `source/env/cheat_file.*` | 旧金手指冲突的**两半判据**:文件名(`IsLegacyCheatFileName`:主模块号前 8 字节的十六进制 + `.txt`,20 字符,大小写都认)与内容(`ChatCodeCheatEntry`:文件里存在同时读 `main+0x05255A60`(文字输入对象)和 `main+0x5474040`(玩家链)的条目)。文件名形状来自 dmnt 的载入路径,内容判据来自 cheat 本身的关键地址:重命名条目不影响识别,而只带玩家链的条目(里程数那条)不会被误报。解析容忍注释、花括号、CRLF 与开头的 UTF-8 BOM(记事本默认就写 BOM,而 BOM 会让第一行不再被当成条目头、连带丢掉那条代码)。纯逻辑,主机侧有回归用例。**这两个地址是版本绑定的**:重生成 cheat(换游戏版本、改偏移)必须同步改常量与夹具,否则检测会静默不再命中 |
...
| `source/ui/app.*` | 页面状态机(首页 / 详情 / 安装确认 / 卸载确认 / **进度** / **聊天码说明** / 结果)、输入处理、卡片式布局与渲染;首页状态由 `ui/home_state.hpp` 的八态分类决定 |
...
- 交互(现状):首页 `A` 执行主操作(安装 / 重装 / 重试 / 已是最新时刷新)、`X` 检查更新、
  ...
  `B` 返回、在状态页再按一次退出;`+`(或页眉第三个入口,与标签共用 `LayoutHeaderTabs`)
  从任何页面打开“聊天码说明”,再按 `+` 或 `B` 回到进来时的那一页。页眉从左到右读作
  `Ⓛ 状态 / Ⓡ 详情 / ＋ 功能说明`;顺序本身就是契约 —— 真机测试靠“哪个入口带下划线”判断当前
  在哪一页,而判定用的是**页面自身特征**(首页两个按钮的徽章、说明页左下那块二维码),不依赖
  入口的位置,所以改名或换语言都不会让它误判。
  安装/确认/进度/聊天码说明/结果/卸载各自一页,共用同一套渲染;说明页里的二维码由 `tools/make-qr.py` 从
  《森友物码册》小程序码采样成模块矩阵(`data/qr_miniapp.bin`),界面用矩形填充逐格画出,主机侧没有图片解码器。
...
- **漏看就会出错的消息留在首页**。首页状态行只讲安装状态,但有一类事实不是状态、却会改变结果:
  游戏目录里还留着会冲突的金手指(见 §6 的 `env/cheat_file.*`:文件是
  `cheats/<主模块号前 8 字节>.txt`,且其中确有一条读文字输入对象的条目)。它和 agent 走同一条
  游戏内输入通路,两个一起用会重复生成物品。这类消息画成卡片区上方的琥珀色警告条,同时仍在详情页
  保留一行 —— 分层规则见 `AGENTS.md` 的文案原则。
- **警告条给绝对路径,不给文件名**。`/atmosphere/contents/01006F8002326000/cheats/…txt` 是玩家
  能自己去找的东西;只给文件名,屏幕上没有任何地方说明它在哪里。路径比名字长 42 个字符,所以这一行
  用 `kFontSmall`(18),而 `kHomeWarnHeight` 与按钮位移 `kHomeWarnShift` 都由行高推导,改字号
  不会让警告条压到按钮。
- **警告条说的是代码,不是文件**。文案是"检测到可能冲突的旧金手指代码" —— 文件躺在卡上没问题
  (§6 的 `env/cheat_file.*` 只在文件里真有那条读文字输入对象的代码时才报),所以要说的对象是代码,
  路径只回答"东西在哪"。颜色是琥珀橙:边框与三角用 `kWarnBright`(装饰,可以亮),文字用 `kWarn`
  (要能在浅色底上读),底色 `kWarnTint` 是淡黄而不是偏红的浅桃色 —— 警告不是错误,红只留给失败。
  这一行**不加**按钮用的 `kLabelOpticalBias`:那个偏置是给 110 px 按钮里的标签校的,用在这里会把
  文字压到三角下方(真机实测前后各量一次,数字见 `docs/device-acceptance.md` 的同名一节:
  加偏置时文字墨迹中心比三角低 4.5 px,去掉后高 1.5 px)。
  读不出内容的金手指文件(超过读取上限或打不开)仍然报警,只是没有条目名 —— 宁可多报一次,
  也不要漏报一个真在冲突的文件。
- **警告条的位移写在 `BuildHomeActions` 里**,不是渲染里:触控目标就是从那边的动作表来的,
  只在渲染里移动会让手指按到旧位置。

### 7.1 绘制与排版约定

- **键徽章里的字母走合成加粗**(`Font::DrawBold`):主机共享字体只有一档字重,18px 的字母在 40px
  圆里显得单薄,于是同一个字形多画一遍(右下各偏 1px、连带对角线共 4 遍)。真机实测白字墨迹
  `Ⓧ` +13%、`Ⓨ` +8%;页眉三个入口(功能说明 / 状态 / 详情)与页脚 `Ⓑ` 用同一函数,风格统一。
...
- **页面装不下时按固定顺序让步**。详情页的收紧阶梯是一张表(`RenderDetails` 的 `LayoutTier`):
  行距 `kRowGap` → 行距 0 → 底部留白 16 → **整页降一档字号**(`kFontSmall`)。加最后这一档
  是因为英文详情页在"覆盖键提示"折成两行、又多了聊天码那几行之后,连 16 px 底部留白都不够:
  宁可整页小一号,也不让最下面那张卡片被裁掉(裁掉的正是给用户看的最后一行)。三张卡片的行间距
  同样在 `kRowGap`(默认 8)与 0 之间自动下调;
...
启动时那一段诊断块同样如此:`gate:` / `manifest:` 两行必须**按这个构建实际携带的清单**来写
(`ReportEnvironment` 解析内嵌发布物再 `Evaluate`)。这两行曾经在发布导入出现之前被写死成
"no embedded release manifest",于是每个真实构建的日志都说 `no-manifest`,而同一个构建的界面上写的是
`supported` —— 报障时读到的是日志,所以宁可多花几行代码解析,也不要让日志与界面各说一套。
