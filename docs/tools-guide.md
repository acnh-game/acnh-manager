...
| `tools/make-icons.py` | 从两张主图生成三种素材:`assets/icon-org.png` → `assets/icon.jpg`(256×256 JPEG,NACP/hbmenu 方图)+ `assets/icon.png`(**256×150** PNG,商店图标:取主图居中一条 900 行缩到 150 高,两侧补天空色,因为客户端详情页那个方框只有 256×150 且不缩放);`assets/screen-org.png`(可选,不入库)→ `assets/screen.png`(**848×208** 商店横幅,按 848:208 居中裁带,绝不拉伸填满;主图不在就跳过)。`--source`/`--banner-source` 可指向别处的主图,`--skip-banner` 跳过横幅;需带 Pillow 的运行时 | 换图标/横幅或商店素材时 |
